...
function nmValidateNewMarks(s,p,beforeCells){
  ...
  if(!before.size&&!added.some(k=>Number(k.split("-")[1])===7))return {ok:false,msg:"Dein erster Zug muss in Spalte H beginnen."};
  if(before.size&&!added.some(k=>nmHasAdjacentMarked(k,[...before])))return {ok:false,msg:"Mindestens ein neues Kreuz muss direkt neben einem bereits angekreuzten Feld liegen."};
