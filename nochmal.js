...
function nmDiceHtml(s){
  ...
  nmEnsureOnlineTurn(s);const me=nmMyPlayer(s),active=s.turn.activePlayerId===me.id,done=s.turn.done.includes(me.id),first3=nmIsFirstThree(s),r=s.turn.reserved;
  const colorDice=colors.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||(!active&&!first3)||!nmAvailableDie(s,"color",i)?'disabled':'')+' class="nm-die nm-color-die '+dieColor(x)+' '+(r?.color===i?'nm-selected':'')+'" data-nm-die-color="'+i+'">'+colorFace(x)+'</button>').join("");
  const numDice=nums.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||(!active&&!first3)||!nmAvailableDie(s,"number",i)?'disabled':'')+' class="nm-die nm-number-die '+(r?.number===i?'nm-selected':'')+'" data-nm-die-number="'+i+'">'+numFace(x)+'</button>').join("");
  let action='';
...
app.addEventListener("click",async e=>{
  ...
  if(e.target.id==="nm-roll-all"){if(s.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s);if(s.turn.activePlayerId!==me.id||s.turn.rolled)return;s.turn.rolled=true;s.turn.reserved=null}s.dice={colors:Array.from({length:3},()=>nmRollDie(NM_DIE_COLORS)),numbers:Array.from({length:3},()=>nmRollDie(NM_DIE_NUMBERS))};nmSave(s,true);const panel=document.querySelector(".nm-dice-panel");if(panel){const temp=document.createElement("div");temp.innerHTML=nmDiceHtml(s);panel.replaceWith(temp.firstElementChild)}return}
  const dc=e.target.closest("[data-nm-die-color]");if(dc&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s);if(s.turn.done.includes(me.id))return;s.turn.choice=s.turn.choice||{};s.turn.choice[me.id]=s.turn.choice[me.id]||{};s.turn.choice[me.id].color=Number(dc.dataset.nmDieColor);if(s.turn.activePlayerId===me.id&&!nmIsFirstThree(s)){s.turn.reserved=s.turn.reserved||{};s.turn.reserved.color=Number(dc.dataset.nmDieColor)}nmSave(s,true);renderNochMal();return}
  const dn=e.target.closest("[data-nm-die-number]");if(dn&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s);if(s.turn.done.includes(me.id))return;s.turn.choice=s.turn.choice||{};s.turn.choice[me.id]=s.turn.choice[me.id]||{};s.turn.choice[me.id].number=Number(dn.dataset.nmDieNumber);if(s.turn.activePlayerId===me.id&&!nmIsFirstThree(s)){s.turn.reserved=s.turn.reserved||{};s.turn.reserved.number=Number(dn.dataset.nmDieNumber)}nmSave(s,true);renderNochMal();return}
  if(e.target.id==="nm-pass"&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s);if(s.turn.activePlayerId!==me.id)return;s.turn.reserved=null;s.turn.done.push(me.id);nmFinishTurnIfReady(s);nmSave(s,true);renderNochMal();return}
  if(e.target.id==="nm-confirm-dice"&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s),ch=s.turn.choice?.[me.id];if(!ch||ch.color==null||ch.number==null)return alert("Bitte einen Farb- und einen Zahlenwürfel auswählen.");if(!nmAvailableDie(s,"color",ch.color)||!nmAvailableDie(s,"number",ch.number))return alert("Diese Würfel hat der aktive Spieler bereits genommen.");if(s.turn.activePlayerId===me.id&&!nmIsFirstThree(s))s.turn.reserved={color:ch.color,number:ch.number};s.turn.done.push(me.id);nmFinishTurnIfReady(s);nmSave(s,true);renderNochMal();return}
