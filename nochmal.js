...
function nmEnsureOnlineTurn(s){if(!s.online)return;s.turn=s.turn||{number:1,activePlayerId:s.players[0]?.id,rolled:false,reserved:null,done:[],choice:{},beforeCells:{}};s.turn.done=Array.isArray(s.turn.done)?s.turn.done:[];s.turn.choice=s.turn.choice||{};s.turn.beforeCells=s.turn.beforeCells||{};if(!s.turn.rolled){s.turn.reserved=null;s.turn.choice={};s.turn.beforeCells={};s.turn.done=[]}}
...
function nmAvailableDie(s,type,i){if(!s.online||!s.turn?.reserved)return true;return s.turn.reserved[type]!==i}
...
function nmHasAdjacentMarked(key,cells){const [r,c]=key.split("-").map(Number),set=new Set(cells);return [[r-1,c],[r+1,c],[r,c-1],[r,c+1]].some(([rr,cc])=>set.has(rr+"-"+cc))}
function nmPlacementAllowed(key,p,beforeCells){
  const before=new Set(beforeCells||[]);
  if(before.has(key))return true;
  if(!before.size)return Number(key.split("-")[1])===7;
  return nmHasAdjacentMarked(key,beforeCells);
}
...
function nmValidateNewMarks(s,p,beforeCells){
  ...
  const startsInH=added.some(k=>Number(k.split("-")[1])===7);
  const connectsToOld=before.size&&added.some(k=>nmHasAdjacentMarked(k,[...before]));
  if(!startsInH&&!connectsToOld)return {ok:false,msg:"Der Zug muss in Spalte H beginnen oder direkt an ein bereits angekreuztes Feld anschließen."};
  const seen=new Set([added[0]]),queue=[added[0]];
  while(queue.length){const cur=queue.shift(),[r,c]=cur.split("-").map(Number);for(const [rr,cc] of [[r-1,c],[r+1,c],[r,c-1],[r,c+1]]){const k=rr+"-"+cc;if(addedSet.has(k)&&!seen.has(k)){seen.add(k);queue.push(k)}}}
  if(seen.size!==added.length)return {ok:false,msg:"Alle neuen Kreuze dieses Zuges müssen direkt zusammenhängen."};
  return {ok:true};
}
function nmFinishTurnIfReady(s){if(!s.online)return;const ids=s.players.map(p=>p.id);if(!ids.every(id=>s.turn.done.includes(id)))return;const ai=ids.indexOf(s.turn.activePlayerId);s.turn={number:s.turn.number+1,activePlayerId:ids[(ai+1)%ids.length],rolled:false,reserved:null,done:[],choice:{},beforeCells:{}};nmPendingDiceChoice={color:null,number:null};s.dice={colors:[null,null,null],numbers:[null,null,null]}}
...
function nmDiceHtml(s){
  ...
  const colorDice=colors.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!r)||(!active&&!nmAvailableDie(s,"color",i))||(x==="joker"&&me.jokers<1+(nums[pending.number]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-color-die '+dieColor(x)+' '+(pending.color===i?'nm-selected':'')+'" data-nm-die-color="'+i+'">'+colorFace(x)+'</button>').join("");
  const numDice=nums.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!r)||(!active&&!nmAvailableDie(s,"number",i))||(x==="joker"&&me.jokers<1+(colors[pending.color]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-number-die '+(pending.number===i?'nm-selected':'')+'" data-nm-die-number="'+i+'">'+numFace(x)+'</button>').join("");
  let action='';
  ...
  else if(!active&&!r) action='<button class="nm-roll-all" disabled>Aktiver Spieler wählt Würfel…</button>';
  ...
  return '<div class="nm-dice-panel"><small>Zug '+s.turn.number+' · '+(active?'Du bist aktiv':escapeHtml(s.players.find(p=>p.id===s.turn.activePlayerId)?.name||"")+' ist aktiv')+''+'</small><div class="nm-dice-row">'+colorDice+'</div><div class="nm-dice-row">'+numDice+'</div>'+action+'</div>';
}
...
function renderNochMal(){
  ...
  '<section class="card nm-jokers"><div><strong>Verbleibende Joker</strong><small>'+(s.online?'Werden beim Einsatz automatisch abgezogen':'Am Spielende je +1 Punkt')+'</small></div><div class="nm-joker-actions">'+(s.online?'<b>'+p.jokers+'</b>':'<button type="button" data-nm-joker="-1">−</button><b>'+p.jokers+'</b><button type="button" data-nm-joker="1">+</button>')+'</div></section>'+
...
app.addEventListener("click",async e=>{
  ...
  if(e.target.id==="nm-online-join"){try{const code=document.querySelector("#nm-room-input")?.value||"",name=document.querySelector("#nm-join-name")?.value.trim()||"";if(!name)return alert("Bitte deinen Namen eingeben.");if(code.trim().length!==4)return alert("Bitte 4-stelligen Raumcode eingeben.");await nmJoinOnline(code,name);renderNochMal()}catch(err){alert(err.message)}return}
  if(e.target.id==="nm-start-online"&&s?.online&&s.phase==="lobby"){const me=nmMyPlayer(s);if(me?.id!==s.hostPlayerId)return;s.phase="playing";s.turn={number:1,activePlayerId:s.hostPlayerId,rolled:false,reserved:null,done:[],choice:{},beforeCells:{}};s.dice={colors:[null,null,null],numbers:[null,null,null]};nmSave(s,true);renderNochMal();return}
  ...
  if(e.target.id==="nm-confirm-dice"&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s),ch={...nmPendingDiceChoice};if(ch.color==null||ch.number==null)return alert("Bitte einen Farb- und einen Zahlenwürfel auswählen.");if(!nmAvailableDie(s,"color",ch.color)||!nmAvailableDie(s,"number",ch.number))return alert("Diese Würfel hat der aktive Spieler bereits genommen.");s.turn.choice=s.turn.choice||{};s.turn.choice[me.id]=ch;s.turn.beforeCells=s.turn.beforeCells||{};s.turn.beforeCells[me.id]=[...me.cells];if(s.turn.activePlayerId===me.id)s.turn.reserved={color:ch.color,number:ch.number};nmPendingDiceChoice={color:null,number:null};nmSave(s,true);renderNochMal();return}
  ...
  const cell=e.target.closest("[data-nm-cell]");if(cell){const p=s.players.find(x=>x.id===s.active),k=cell.dataset.nmCell;if(s.online){const me=nmMyPlayer(s),ch=nmResolvedChoice(s,me.id);if(!ch)return alert("Bitte zuerst Würfel auswählen und bestätigen.");const vals=nmChoiceValues(s,ch);if(!nmCellAllowedForChoice(k,vals,me))return alert("Dieses Feld passt nicht zu deinem gewählten Farbwürfel.");}const i=p.cells.indexOf(k);if(i>=0)p.cells.splice(i,1);else {if(s.online){const before=s.turn?.beforeCells?.[p.id]||[],newThisTurn=p.cells.filter(x=>!new Set(before).has(x));if(!newThisTurn.length&&Number(k.split("-")[1])!==7&&!nmHasAdjacentMarked(k,before))return alert("Beginne in Spalte H oder direkt neben einem bereits angekreuzten Feld.");if(newThisTurn.length&&!nmHasAdjacentMarked(k,newThisTurn)&&!(before.length&&nmHasAdjacentMarked(k,before)))return alert("Die neuen Kreuze dieses Zuges müssen direkt zusammenhängen.");}p.cells.push(k);}nmRecalcClaims(s);nmSave(s,false);cell.classList.toggle("checked",i<0);const score=nmScore(s,p);const total=document.querySelector(".nm-total strong");if(total)total.textContent=score.total;const activeTab=document.querySelector(".nm-tab.active strong");if(activeTab)activeTab.textContent=score.total;const vals=document.querySelectorAll(".nm-summary strong");if(vals.length>=4){vals[0].textContent=score.colPts;vals[1].textContent=score.colorPts;vals[2].textContent=p.jokers;vals[3].textContent="−"+(score.openStars*2)}return}
  if(e.target.id==="nm-confirm-fields"&&s?.online){nmEnsureOnlineTurn(s);const me=nmMyPlayer(s),before=s.turn.beforeCells?.[me.id]||[];const check=nmValidateNewMarks(s,me,before);if(!check.ok)return alert(check.msg);const ch=nmResolvedChoice(s,me.id),vals=nmChoiceValues(s,ch),jokerCount=(vals?.color==="joker"?1:0)+(vals?.number==="joker"?1:0);if(jokerCount>me.jokers)return alert("Du hast nicht mehr genug Joker.");me.jokers-=jokerCount;if(!s.turn.done.includes(me.id))s.turn.done.push(me.id);nmFinishTurnIfReady(s);nmSave(s,true);renderNochMal();return}
