...
function nmDiceHtml(s){
  ...
  const colorDice=colors.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!r)||!nmAvailableDie(s,"color",i)||(x==="joker"&&me.jokers<1+(nums[pending.number]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-color-die '+dieColor(x)+' '+(pending.color===i?'nm-selected':'')+'" data-nm-die-color="'+i+'">'+colorFace(x)+'</button>').join("");
  const numDice=nums.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!r)||!nmAvailableDie(s,"number",i)||(x==="joker"&&me.jokers<1+(colors[pending.color]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-number-die '+(pending.number===i?'nm-selected':'')+'" data-nm-die-number="'+i+'">'+numFace(x)+'</button>').join("");
  let action='';
...
app.addEventListener("click",async e=>{
  ...
  if(e.target.id==="nm-start-online"&&s?.online&&s.phase==="lobby"){const me=nmMyPlayer(s);if(me?.id!==s.hostPlayerId)return;s.phase="playing";s.turn={number:1,activePlayerId:s.hostPlayerId,rolled:false,reserved:null,done:[]};s.dice={colors:[null,null,null],numbers:[null,null,null]};nmSave(s,true);renderNochMal();return}
