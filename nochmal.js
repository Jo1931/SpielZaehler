...
function nmDiceHtml(s){
  ...
  const colorDice=colors.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!first3)||!nmAvailableDie(s,"color",i)||(x==="joker"&&me.jokers<1+(nums[pending.number]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-color-die '+dieColor(x)+' '+(pending.color===i?'nm-selected':'')+'" data-nm-die-color="'+i+'">'+colorFace(x)+'</button>').join("");
  const numDice=nums.map((x,i)=>'<button type="button" '+(!s.turn.rolled||done||confirmed||(!active&&!first3)||!nmAvailableDie(s,"number",i)||(x==="joker"&&me.jokers<1+(colors[pending.color]==="joker"?1:0))?'disabled':'')+' class="nm-die nm-number-die '+(pending.number===i?'nm-selected':'')+'" data-nm-die-number="'+i+'">'+numFace(x)+'</button>').join("");
  let action='';
